...
def load_data_from_gcs(data_gcs_path: str) -> pd.DataFrame:
    ...
    # -- 1. Tutorial (Dask) approach --
    # return dd.read_csv(data_gcs_path).compute()
    
    # -- 2. Service Account Key approach --
    # dir_path = os.path.dirname(os.path.realpath(__file__))
    # key_file_path = os.path.join(dir_path, "vertex-ai-371116-c1cae6a617bc.json")
    # key_file_path = os.path.join('/mnt', 'vertex-ai-371116-c1cae6a617bc.json')
    # with open(key_file_path, 'r') as t:
    #     token = json.load(t)
    # credentials = service_account.Credentials.from_service_account_file("trainer/vertex-ai-371116-c1cae6a617bc.json")
    # client = storage.Client(project='vertex-ai-371116', credentials=credentials)
    
    # -- 3. GCSFS approach --
    # fs = gcsfs.GCSFileSystem(project='vertex-ai-371116', token=token)
    # fs = gcsfs.GCSFileSystem(project='vertex-ai-371116')
    # with fs.open(data_gcs_path) as f:
    #     df = pd.read_csv(f, encoding='utf-8')

    # -- 4. Pandas direct path --
    # return pd.read_csv(data_gcs_path, encoding='utf=8')
    
    # -- 5. Blob storage --
#     client = storage.Client(credentials=credentials, project='vertex-ai-371116')
    client = storage.Client(project='vertex-ai-371116')
    bucket = client.bucket("datasetsgcp")
    blob = bucket.blob("prebuilt-container-method/titanic-encoded.csv")
...
#         .to_dataframe(bqstorage_client=bqstorageclient)
    # return pd.read_gbq(query_string, project_id=project[5:])
    credentials, project_id = google.auth.default()
    client = storage.Client(credentials=credentials, project=project_id)
    bucket = client.bucket("datasetsgcp")
    blob = bucket.blob("titanic/prebuilt-container-method/titanic.csv")
    return
    return pd.read_csv(blob.download_as_string().decode('utf-8'))
...
def pipeline_export_gcs(fitted_pipeline: Pipeline, model_dir: str) -> str:
    ...
    # logging.info(scheme, bucket, path, file)
    # if scheme != "gs:":
    #         raise ValueError("URI scheme must be gs")
    
    # Upload the model to GCS
    bkt = storage.Client(project='vertex-ai-371116').bucket(bucket)
    export_path = os.path.join(path, 'model.pkl')
    logging.info(export_path)
    blob = bkt.blob(export_path)
    
    blob.upload_from_string(pickle.dumps(fitted_pipeline))
    return "gs://" + os.path.join(bucket, export_path)
...
def report_export_gcs(report: str, report_dir: str) -> None:
    ...
    if str(bucket):
        ...
        raise ValueError("URI scheme must be string")
